...
public class DrivePastChargeStationTaxiReverse extends CommandBase {
Translation2d translation;
Drive drive;
double lowestRoll = 0;
double highestRoll = 0;
  // must have back facing charge station when initialized
  public DrivePastChargeStationTaxiReverse(Drive drive) {
  ...
  public boolean isFinished() {
   if(drive.gyro.getRoll() > -5 && drive.gyro.getRoll() < 5 && lowestRoll < -13 && highestRoll > 13) {
    new WaitCommand(.5);
    return true;
   }
   else {
